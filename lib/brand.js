...
/**
 * Identifica cuentas organizacionales con acceso incluido a los módulos de la plataforma.
 * Se configura con FREE_ACCESS_DOMAINS y mantiene el dominio corporativo actual como respaldo.
 */
export function isOrganizationEmail(email) {
  if (!email || typeof email !== 'string') return false;

  const clean = email.trim().toLowerCase();
  const configuredDomains = process.env.FREE_ACCESS_DOMAINS || 'tsolutionsipidd.com';
...
export function generateDeliveryInstructions({ nombre, apellido, empresa, slug, originUrl }) {
  ...
  return `======================================================================
🌐 ENLACE CLOUD OFICIAL DE TU IDENTIDAD DIGITAL INTERACTIVA
======================================================================
👉 TU ENLACE CLOUD PÚBLICO (COMPARTE ESTE LINK):
   ${enlacePerfil}

👉 TU PORTAL PERSONAL DE GESTIÓN Y TELEMETRÍA:
   ${originUrl || website}/portal?slug=${slug}
======================================================================
