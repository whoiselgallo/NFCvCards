  ...
  try {
    await p.query(`
      CREATE TABLE IF NOT EXISTS vcard_profiles (
        ...
        empresa VARCHAR(150),
        puesto VARCHAR(150),
        cedula_profesional VARCHAR(180) DEFAULT '',
        permisos_profesionales TEXT DEFAULT '',
        telefono VARCHAR(50),
        whatsapp VARCHAR(50),
        correo VARCHAR(150),
        url VARCHAR(255),
      ...
      ALTER TABLE vcard_profiles ADD COLUMN IF NOT EXISTS cedula_profesional VARCHAR(180) DEFAULT '';
      ALTER TABLE vcard_profiles ADD COLUMN IF NOT EXISTS permisos_profesionales TEXT DEFAULT '';
      ...
      ALTER TABLE vcard_profiles ADD COLUMN IF NOT EXISTS user_id INTEGER REFERENCES users(id) ON DELETE CASCADE;

      -- Tabla de Organizaciones (Multi-Tenant B2B & RBAC)
      CREATE TABLE IF NOT EXISTS organizations (
        id SERIAL PRIMARY KEY,
        name VARCHAR(255) NOT NULL,
        slug VARCHAR(100) UNIQUE NOT NULL,
        custom_domain VARCHAR(255),
        primary_color VARCHAR(20) DEFAULT '#ff0003',
        logo_url TEXT,
        enforce_branding BOOLEAN DEFAULT false,
      ...
      ALTER TABLE vcard_profiles ADD COLUMN IF NOT EXISTS organization_id INTEGER REFERENCES organizations(id) ON DELETE SET NULL;

      -- Tabla de Webhooks para Integraciones (Zapier, HubSpot, Salesforce, Make)
      CREATE TABLE IF NOT EXISTS webhooks (
        id SERIAL PRIMARY KEY,
        name VARCHAR(255) NOT NULL,
        url TEXT NOT NULL,
        event_type VARCHAR(50) DEFAULT 'lead_submit', -- 'lead_submit', 'card_event', '*'
      ...
      -- Tabla de Telemetría Estructurada por Origen (NFC vs QR vs Link Directo)
      CREATE TABLE IF NOT EXISTS card_events (
        id SERIAL PRIMARY KEY,
        profile_id INT REFERENCES vcard_profiles(id) ON DELETE CASCADE,
        slug VARCHAR(255) NOT NULL,
        organization_id INT NULL,
        source_type VARCHAR(50) NOT NULL DEFAULT 'direct', -- 'nfc', 'qr', 'direct', 'wallet', 'share'
        event_type VARCHAR(50) NOT NULL, -- 'page_view', 'vcard_download', 'whatsapp_click', 'lead_submit', 'map_click'
        user_agent TEXT,
        ip_address VARCHAR(100),
        country_code VARCHAR(10),
        device_type VARCHAR(50), -- 'mobile', 'desktop', 'tablet', 'watch'
        created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
      );
      CREATE INDEX IF NOT EXISTS idx_card_events_slug_created ON card_events(slug, created_at DESC);
      CREATE INDEX IF NOT EXISTS idx_card_events_source ON card_events(source_type, event_type);

      -- Tabla de Captura de Leads Bidireccionales
      ...
      CREATE INDEX IF NOT EXISTS idx_card_leads_slug ON card_leads(slug, created_at DESC);
      ...
      -- Columna password_hash en users (por si no existe)
